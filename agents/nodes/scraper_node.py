...
def run_universal_scraper(business_name: str, location: str, latitude: float = None, longitude: float = None) -> List[Dict[str, Any]]:
    ...
    For hotels: Uses Booking.com automated search
    For other businesses: Uses Serper API + general scraping
    ...
    Returns:
        List of scraped business data (passed to LLM for analysis)
    """
    try:
        ...
        # Check if this is a hotel query - use Booking.com automated search (NO SERPER NEEDED)
        if "hotel" in business_name.lower():
            logger.info(f"🏨 Detected hotel query - using Booking.com direct scraper")
            logger.info("ℹ️  No Serper API needed - going directly to Booking.com")
            search_location = location
            
            # Use the new scrape_booking_hotels function - goes directly to Booking.com
            scraped_data = universal_scrapper.scrape_booking_hotels(search_location, headless=True)
            
            logger.info(f"✅ Scraped {len(scraped_data)} hotels from Booking.com")
            logger.info("📊 Data will be passed to LLM for pros/cons analysis")
            return scraped_data
        
        # For other businesses, try Serper API if available, otherwise skip
        logger.info(f"⚠️  Non-hotel business detected: {business_name}")
        logger.info("ℹ️  Serper API would be needed for automated URL discovery")
        logger.info("ℹ️  Skipping scraper for non-hotel businesses (only hotels supported)")
        
        # Return empty list - other business types handled by existing scrapers
        # (cafes/restaurants use swiggy_scraper.py, schools use school_scrapper.py)
        return []
